...
import tide.trader.bot.dto.position.PositionCloseResultDTO;
...
public abstract class CassandreStrategyImplementation extends BaseStrategy implements CassandreStrategyInterface {
    ...
    public final void setDependencies(final CassandreStrategyDependencies cassandreStrategyDependencies) {
        ...
        dependencies.getPositionRepository()
                .findByStatusNot(PositionStatusDTO.CLOSED)
    ...
    public final void ordersUpdates(final Set<OrderDTO> orders) {
        ...
        final Map<String, OrderDTO> ordersUpdates = orders.stream()
                ...
                //.peek(orderDTO -> dependencies.getMessageService().orderUpdateMessage((CassandreStrategy) this, orderDTO))
        ...
        positions.stream()
                ...
                .forEach(positionDTO -> {
                    ...
                    if (previousPositionsStatus.get(positionDTO.getUid()) != positionDTO.getStatus()) {
                        if (positionDTO.getStatus() == PositionStatusDTO.CLOSED) {
    ...
    protected void updatePositionsWithTickersUpdates(final Map<CurrencyPairDTO, TickerDTO> tickers) {
        ...
        dependencies.getPositionRepository()
                ...
                //.filter(PositionDTO::shouldBeClosed1)
                .peek(positionDTO -> logger.debug("Closing position {}", positionDTO.getPositionId()))
                .map(PositionDTO::shouldBeClosed)
                .filter(PositionCloseResultDTO::isClose)
                .map(positionCloseResult -> dependencies.getPositionService().closePosition(this, positionCloseResult.getPosition().getUid(), tickers.get(positionCloseResult.getPosition().getCurrencyPair()), positionCloseResult.getPosition().getExitReason()))
    ...
    void updatePositionsWithOrdersUpdates(final Map<String, OrderDTO> orders) {
        ...
        orders.values().forEach(orderDTO -> dependencies.getMessageService().orderUpdateMessage((CassandreStrategy) this, orderDTO));
    }
