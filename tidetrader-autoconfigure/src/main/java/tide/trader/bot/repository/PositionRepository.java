...
public interface PositionRepository extends JpaRepository<Position, Long>, JpaSpecificationExecutor<Position> {
    ...
    /**
     * Retrieve by strategyUid positions (sorted by uid).
     * @param strategyUid
     * @return
     */
    @Query("select p from Position p where p.strategy.uid = :strategyUid order by p.uid")
    List<Position> findByStrategyUid(@Param("strategyUid") Long strategyUid);
     ...
     * @param exitReason Reason for closing position
     */
    @Transactional
    @Modifying
    @Query("update Position p set p.forceClosing = :value, p.exitReason = :exitReason where p.uid = :uid")
    void updateForceClosing(@Param("uid") Long uid, @Param("value") boolean value, @Param("exitReason") String exitReason);
