...
public class Band2SDLowerIndicator extends CachedIndicator<Num> {

    private final SMAIndicator sma;
    private final StandardDeviationIndicator stdev;
    private final double b2mult;

    public Band2SDLowerIndicator(Indicator<Num> indicator, double b2mult, int barCount) {
        super(indicator);
    ...
    protected Num calculate(int i) {
        return sma.getValue(i).minus(this.numOf(b2mult).multipliedBy(stdev.getValue(i)));
    }
}
