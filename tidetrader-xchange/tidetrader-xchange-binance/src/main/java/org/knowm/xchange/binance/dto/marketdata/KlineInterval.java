...
public enum KlineInterval {
  ...
  public static KlineInterval lookup(long millis) {
    Optional<KlineInterval> optional = Arrays.stream(KlineInterval.values()).filter(interval -> interval.getMillis() == millis).findFirst();
    if(optional.isEmpty()) {
      return null;
    }
    return optional.get();
